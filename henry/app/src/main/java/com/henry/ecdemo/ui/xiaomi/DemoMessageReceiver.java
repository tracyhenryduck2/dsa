package com.henry.ecdemo.ui.xiaomi;
...
public class DemoMessageReceiver extends PushMessageReceiver {
    ...
    public void onReceivePassThroughMessage(Context context, MiPushMessage message){
        ...
        msg.obj = log;
        ECApplication.getHandler().sendMessage(msg);
    }
    
    @Override
    public void onNotificationMessageClicked(Context context, MiPushMessage message){
        Log.v(ECApplication.TAG,
                "onNotificationMessageClicked is called. " + message.toString());
        String log = context.getString(R.string.click_notification_message, message.getContent());
        MainActivity.logList.add(0, getSimpleDate() + " " + log);

           Log.e("aaaa", message.getExtra().get("msgDomain"));
           Log.e("aaaa", message.getExtra().get("ecMessage"));
    ...
    public void onCommandResult(Context context, MiPushCommandMessage message) {
        ...
        if (MiPushClient.COMMAND_REGISTER.equals(command)) {
            if (message.getResultCode() == ErrorCode.SUCCESS) {
                ...
                CCPAppManager.put("xiaomi",mRegId);
